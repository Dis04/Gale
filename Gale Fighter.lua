...
if getgenv().Reanimation == "PermaDeath" or getgenv().Reanimation == nil then
	...
	loadstring(game:HttpGet("https://raw.githubusercontent.com/h17s3/Reanimations/main/Reanimation%20God"))()
	task.wait()
...
Dead = false
spawn(function()
	repeat wait() until not workspace:FindFirstChild("non")
	Dead = true
end)
cool = false
local FakeMouse = game.Players.LocalPlayer:GetMouse()
FakeMouse.Button1Down:Connect(function()
	if Dead == false then
		if cool == false then
			cool = true
...
cd = false
FakeMouse.KeyDown:Connect(function(k)
	if Dead == false and k then
		if string.lower(k) == "r" then
			if cd == false then
				cd = true
				wait(.8)
				for i = 1,30 do
					wait(.112345656354)
					for i2,v in pairs(game.Players:GetPlayers()) do
						local A_1 = v
						local Event = game:GetService("ReplicatedStorage").meleeEvent
						spawn(function()
							Event:FireServer(A_1)
